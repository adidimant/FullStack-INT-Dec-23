...
// Function to check if a username or email already exist in the table
...
function validateCity() {
    ...
    if (cityInput.value.length < 1) {
        cityError.textContent = 'The first name must contain at least 1 character.';
    } else {
...
/*function validateFname() {
    const fnameInput = document.getElementById('fname');
    const fnameError = document.getElementById('phoneError');
...
function submitForm(event) {
    ...
    // Run validation once more before submitting
    validate();

    // Check if there are any error messages
    if (emailError.textContent || phoneError.textContent || usernameError.textContent) {
