...
const registeredFilterStart = document.querySelector(
  "#registeredFilterStart"
) as HTMLInputElement;
const registeredFilterEnd = document.querySelector(
  "#registeredFilterEnd"
) as HTMLInputElement;
const updatedFilterStart = document.querySelector(
  "#updatedFilterStart"
) as HTMLInputElement;
const updatedFilterEnd = document.querySelector(
  "#updatedFilterEnd"
) as HTMLInputElement;
...
function dateFilterChangeStart(e: Event) {
  ...
  const newUsers = currentlyShowedUsers.filter(
    (user: userDetails) => Number(user.registeredDate) >= timeStampDate
  );
...
function dateFilterChangeEnd(e: Event) {
  ...
  const newUsers = usersFromDBCopy.filter(
    (user: userDetails) => Number(user.registeredDate) <= timeStampDate
  );
  deleteUsersFromTable();
  loadTableFromUsersArray(newUsers);
...
function inputsFilter(e: Event) {
  setTimeout(() => {
    ...
    switch ((e.target as HTMLInputElement).id) {
      case "userNameFilter":
        newUsers = currentlyShowedUsers.filter((user: userDetails) =>
      ...
      case "firstNameFilter":
        newUsers = currentlyShowedUsers.filter((user: userDetails) =>
          user.firstName.toLocaleLowerCase().includes(input.toLocaleLowerCase())
        );

        break;
      case "lastNameFilter":
        newUsers = currentlyShowedUsers.filter((user: userDetails) =>
          user.lastName.toLocaleLowerCase().includes(input.toLocaleLowerCase())
        );
        break;
      case "countryFilter":
        newUsers = currentlyShowedUsers.filter((user: userDetails) =>
          user.country.toLocaleLowerCase().includes(input.toLocaleLowerCase())
        );
        break;
      default:
          ...
          if (!closestTr.querySelector("input")) {
            tdsDetails.forEach((element) => {
              ...
              element.appendChild(editInput);
						...
						user.userName = tdsDetails[userEnum.userName].querySelector("input")!.value ?? ""
						user.email = tdsDetails[userEnum.email].querySelector("input")!.value ?? ""
						user.password = tdsDetails[userEnum.password].querySelector("input")!.value ?? ""
						user.phoneNumber = tdsDetails[userEnum.phoneNumber].querySelector("input")!.value ?? ""
						user.firstName = tdsDetails[userEnum.firstName].querySelector("input")!.value ?? ""
						user.lastName = tdsDetails[userEnum.lastName].querySelector("input")!.value ?? ""
						user.country = tdsDetails[userEnum.country].querySelector("input")!.value ?? ""
						user.city = tdsDetails[userEnum.city].querySelector("input")!.value ?? ""
						user.zipCode = tdsDetails[userEnum.zipCode].querySelector("input")!.value ?? ""
						console.log(user)
						deleteUsersFromTable()
...
function removeAushUserIDToLocalStorage(
  usersArray: string[],
  removeUserID: string
) {
