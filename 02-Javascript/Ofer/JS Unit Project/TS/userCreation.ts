...
function submitForm(event: SubmitEvent) {
    ...
    if (checkIfUserInDB(usersIDArrayFromDB, userDetail.userName)) {
        ...
        alert(
            `Password is not strong enough, Please make sure it has at least 1 upperCase letter and a special character`
        );
    } else if (phoneValidation(userDetail.phoneNumber)) {
        alert(
            `phone number is not valid, please make sure it's the correct length and starts with '05'`
        );
    } else if (checkIfPhoneInDB(usersFromDB, userDetail)) {
        alert(
            `Phone Number "${userDetail.phoneNumber}" is already taken, please select a different Phone Number`
        );
    } else {
...
function checkIfPhoneInDB(
    usersArrayFromDB: userDetails[],
    newUser: userDetails
) {
    return usersArrayFromDB.some(
        (user: userDetails) => user.phoneNumber === newUser.phoneNumber
    );
}
...
function checkIfEmailInDB(
    usersArrayFromDB: userDetails[],
    newUser: userDetails
) {
    return usersArrayFromDB.some(
        (user: userDetails) => user.email === newUser.email
    );
}
...
function pushUserDBToLocalStorage(
