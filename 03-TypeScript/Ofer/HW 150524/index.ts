type quadrantPossibilities = 1 | 2 | 3 | 4 | 'onXAxis' | 'onYAxis';
...
class PT {
  ...
  getQuadrant(): quadrantPossibilities {
    if (this.y === 0) return 'onXAxis';
    else if (this.x === 0) return 'onYAxis';
  ...
  calculateDistance(x: number | PT, y?: number): number {
  ...
  calculateIncline(x: number | PT, y?: number): number {
    if (y && typeof x == "number") {
  ...
  calculateMiddlePoint(x: number | PT, y?: number): PT | 0 {
    if (y && typeof x == "number") {
