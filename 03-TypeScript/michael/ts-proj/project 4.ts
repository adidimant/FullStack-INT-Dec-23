...
class Utils {
    static maintainLastXItems<T>(arr: T[], maxSize: number, newItem: T) {
 ...
 class Config {
    ...
    SDK_ENABLED: boolean;
     constructor(interval: number, bufferSize: number, sdkEnabled: boolean){
...
// for some reason when i try to make a class EVENT MANAGER and i put the GET CONFIG METHOD inside of it its shows erros wich i wasnt able to solve .
  function getConfig(customerId: string): Config {
...
class ScreenWidthCollector<T> implements Collector<T>{
    interval: number;
    data: T | null;
    key: string;
    runningInterval: any
    ...
    constructor(interval: number, key: string){
...
class ScreenLenthCollector<T> implements Collector<T>{
    interval: number;
    data: T | null;
    key: string;
    runningInterval: any
...
function main(){
    ...
    let sceenWith = new ScreenWidthCollector<string>(config.COLLECTORS_INTERVAL, 'screenWidth')
    let screenLength = new ScreenLenthCollector<string>(config.COLLECTORS_INTERVAL, 'screenLength')
