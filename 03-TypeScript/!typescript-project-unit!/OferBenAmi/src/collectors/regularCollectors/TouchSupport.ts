import { Collector, dataAndTime } from "../../interfaces";
import { EventsManager } from "../../classes";

export class TouchSupport implements Collector<dataAndTime> {
	public interval: number;
	private data: dataAndTime[] = [];
	constructor(interval?: number) {
		this.interval = interval ?? EventsManager.getConfig().COLLECTORS_INTERVAL;
	}

	public getData() {
		return this.data.length > 0 ? this.data : null;
	}
	public getKey() {
		return `TouchSupport`;
	}
	public startCollect() {
		const touch = 'ontouchstart' in window || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0
		this.data.push([touch, Date.now()]);
		(document.querySelector('#TouchSupport-div') as HTMLDivElement).textContent = String(touch) ?? "";

	}
	public finishCollect() {}
}
