...
type IfObject<T, K> = T extends object ? T : K;

function returnObjectOrElse<T, K>(value: T, fallback: K): IfObject<T, K> {
  if (typeof value === 'object' && value !== null) {
    return value as IfObject<T, K>;
  } else {
    return fallback as IfObject<T, K>;
  }
}
...
function mergeObjects<T extends object, K extends object>(obj8: T, obj9: K): T & K {
  ...
  // הגדרת סוגי הטיפוסים לאובייקטים
  type Obj1Type = { a: number; b: number };
  type Obj2Type = { c: number; d: number };
  
  const obj8: Obj1Type = { a: 1, b: 2 };
  const obj9: Obj2Type = { c: 3, d: 4 };
  const merged = mergeObjects(obj1, obj9);
  console.log(merged); // פלט: { a: 1, b: 2, c: 3, d: 4 }
  
  // הגדרת סוגי הטיפוסים לאובייקט של אדם ולפרטיו
  type PersonType = { name: string };
  type DetailsType = { age: number; city: string };
  
  const person2: PersonType = { name: 'Alice' };
  const details: DetailsType = { age: 25, city: 'Wonderland' };
  ...
  //Answer in class //
  function mergeObjects1<T , K>(obj1: T, obj2: K): T & K {
    // שימוש באופרטור ההתפשטות כדי לשלב את obj1 ו-obj2
    return { ...obj1, ...obj2 };
  }
...
interface MachineInterface {
  ...
  class Machine1 implements MachineInterface {
...
function myLogger<T, K>(param1: T, param2: T, param3: K) {
  ...
  myLogger<string, number>('Ziv', 'Elharar', 100);