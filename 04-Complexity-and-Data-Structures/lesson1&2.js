...
function findCommonItemImproved(arr1, arr2) {
...
function searchItemInArray(arr, item) {
...
function printAllItemsInArray(arr) {
    for (let i = 0; i < arr.length; i++) { // O(n)
        console.log(arr[i]);
    }
}
...
function printAllItemsInArray2(arr) {
    return __awaiter(this, void 0, void 0, function* () {
        for (let i = 0; i < arr.length; i++) { // O(n)
            console.log(arr[i]);
        }
        const newItemsResponse = yield fetch("https://my-server.com/api/getKNewItems"); // O(1)
        const newItems = yield newItemsResponse.json(); // items - array of length k. // O(k)?
...
function printAllItemsInArray3(arr) {
...
//[1000,6,7,8,5,4,6,7]
//[-7-,-9,-20,-11]
function findMaxTenElementsV2(arr) {
    if (arr.length > 10) { // O(1)
        const biggestIndexes = []; // the max size of this array is - 10   // O(1)
        const maxItems = []; // O(1)
