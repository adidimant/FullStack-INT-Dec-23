...
function CreatePostPopup( { show, onClose }: CreatePostPopupProps ) {
  ...
  const handleSharePost = async () => {
    ...
    return (
    <div className="popup-overlay" style={{backgroundColor: theme === 'dark' ? 'rgba(0, 0, 0, 0.7)' : undefined}}>
      <form>
      <div className="popup-window">
      <div className={`headerCreateContainer ${theme}-background`}>
        <div className="closeOrSave" style={{color: theme === 'dark' ? '#ffffff':undefined}} onClick={onClose}>X</div>
				<div>Create new post</div>
        <div className="closeOrSave" style={{color: theme === 'dark' ? '#ffffff':undefined}} typeof="submit" onClick={handleSharePost}>Share</div>
			</div>
      <div className={`CreateContainer ${theme}-background`}>
          <div className="UploadingPicture">
            ...
            <input style={theme === 'dark' ? {border:'none', backgroundColor: '#181818', color:'#ffffff', cursor: 'pointer'} : undefined} type="button" value="Select from computer" onClick={()=> document.getElementById('fileInput')?.click()}></input>
              <input
                id="fileInput"
                type="file"
                accept="image/*"
          ...
          <div className="ContentDetails">
              <div className="profil">
                ...
                <div className="userName">Ofer134</div></div>
                  <label style={{color: theme === 'dark' ? '#ffffff':undefined}}>Description:</label>
                  <textarea className={`textContent ${theme}-background`} style={{backgroundColor: theme === 'dark' ? '#222222':undefined}}
                  rows={4} 
                  value={text}
                  onChange={handleDescriptionChange}
                  maxLength={maxChars}></textarea>
                <div className="imojiAndMaxChars">
                  <div className="emoji"><button onClick={() => setShowEmojiPicker((prev) => !prev)} style={{backgroundColor: theme === 'dark' ? '#181818': undefined}}>
            ...
            <label style={{color: theme === 'dark' ? '#ffffff':undefined}}>Location:</label>
            <input style={ theme === 'dark' ? {border: 'none' , backgroundColor: '#222222'}: undefined} className={`${theme}-background`} type="text" value={location} onChange={handleLocationChange} name="post-location" id="post-location" />
          </div>
